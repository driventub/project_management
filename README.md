...
*(el diagrama entidad-relación se agrega en esta sección — generado aparte a partir de las migraciones de EF Core)*

Entidades y relaciones, para referencia mientras se arma el diagrama: `Usuario 1—N Tarea` (vía `Tarea.ResponsableId`, nullable), `Proyecto 1—N Columna` (vía `Columna.ProyectoId`), `Columna 1—N Tarea` (vía `Tarea.ColumnaId`). `Proyecto.Estado` y `Tarea.Prioridad` son enums almacenados como string (`HasConversion<string>()`).

## Verificación End-to-End Pendiente

No se ejecutó ningún comando desde este entorno (por diseño del proyecto — Claude Code solo edita archivos). Falta correr y confirmar:

```bash
docker compose up --build
```

y click-through completo por http://localhost:8081 (login, CRUD, tablero con drag & drop, tiempo real en dos pestañas, export PDF/Excel) — es la primera vez que se ejercita la app completa a través de nginx en vez de `ng serve`.
